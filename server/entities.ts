...
const LAIRS: LairSpec[] = [
  ...
  // The boss: two lone Runebound Golems, a long trek into the peaks.
  { monster: "golem", lairs: 2, packMin: 1, packMax: 1, minDist: 70, maxDist: 140 },
];
...
export function spawnWorldEntities(world: World): WorldEntities {
  ...
  let made = 0;
  const addMonster = (def: MonsterDef, pos: Vec3): void => {
    const id = `m${++made}`;
    monsters.set(id, {
      id,
      def,
  ...
  for (const spec of LAIRS) {
    ...
    for (let l = 0; l < spec.lairs; l++) {
      ...
      for (let k = 0; k < pack; k++) {
        ...
        if (pos) addMonster(def, pos);
      }
    }
  }

  // Guarantee the boss exists even if no mountains fall in its distance band:
  // spiral outward until mountain terrain turns up.
  let golems = [...monsters.values()].filter((m) => m.def.id === "golem").length;
  for (let r = 40; r <= 400 && golems < 1; r += 12) {
    for (let a = 0; a < Math.PI * 2 && golems < 1; a += Math.PI / 10) {
      const x = Math.round(Math.cos(a) * r);
      const z = Math.round(Math.sin(a) * r);
      if (world.biomeAt(x, z) !== Biome.Mountains) continue;
      const pos = surfacePos(world, x, z);
      if (pos.y <= 1) continue;
      addMonster(MONSTERS.golem, pos);
      golems++;
    }
  }
